...
void MainGameEntry(PLAY_IGNORE_COMMAND_LINE)
{
	...
	Play::CentreAllSpriteOrigins(); //holds each sprite from its centre
	Play::LoadBackground("Data\\Backgrounds\\background.png");
	Play::StartAudioLoop("music");
	Play::CreateGameObject(TYPE_AGENT8, { 115, 0 }, 50, "agent8"); // TYPE_AGENT8: creates Agent 8 object ; { 115, 0 }: sets inital position ; 50: collision radius in pixelsl ; agent8: tells Playermanager which sprite to use for this object
...
{
	Play::DrawBackground(); //replacing Play::ClearDrawingBuffer as both completely reset the display buffer
	UpdateAgent8(); // replacing HandlePlayerControls();
	UpdateFan();
	UpdateTools();
	UpdateCoinsAndStars();
	UpdateLasers();
...
void HandlePlayerControls()
{
	...
	{
		obj_agent8.velocity = { 0, -10 }; // -4: upwards - changed to -10 to increase velocity
		Play::SetSprite(obj_agent8, "agent8_climb", 0.25f); //changing the object's sprite to climbing animation and setting animation speed
	}
	...
	else
	{
		...
		{
			Play::SetSprite(obj_agent8, "agent8_hang", 0.02f); //when neither keys are pressed
			obj_agent8.velocity *= 0.5f; //slowing the velocity bu multiplying by fraction
			obj_agent8.acceleration = { 0, 0 };
		}
	}
	if (Play::KeyPressed(VK_SPACE))
	{
		Vector2D firePos = obj_agent8.pos + Vector2D(155, -75); //appear 155 pixels across and 75 pixels up from agent 8's position
...
void UpdateFan()
{
	...
	if (Play::RandomRoll(50) == 50)
	{
		...
		obj_tool.velocity = Point2f(-8, Play::RandomRollRange(-1, 1) * 6); // varying the tools direction to get random between -1 to 1 and multiplying it by 6 to produce y-axis velocity
...
void UpdateCoinsAndStars()
{
	...
	for (int id_coin : vCoins)
	{
		...
		{
			for (float rad{ 0.25f }; rad < 2.0f; rad += 0.5f) //angles are measures in radians rather than degrees
			{
				...
				Play::SetGameObjectDirection(obj_star, 16, rad * PLAY_PI); //multiplying by Pi t get values in radians
			}
...
void UpdateAgent8()
{
	...
	case STATE_DEAD:
		obj_agent8.acceleration = { -0.3f , 0.5f }; // -0.3f , 0.5f
